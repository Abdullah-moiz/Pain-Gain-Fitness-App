...
export default function PersonalTrainerForm() {
  ...
  const [data, setData] = useState({ name: "", email: "", img: "", address: "", city: "", country: "", state: "", zipcode: "", availableStart : "" , availableEnd : "" })

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:8000/saveTrainerForm' , data).then(res => swal("Done!" , `${res.data.msg}`, "success"))
    } catch (error) {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} method="post" className='flex flex-col text-black w-full items-center justify-center h-4/5'>
        ...
        <div className='w-96   flex items-start flex-col justify-center p-2 m-2'>
          <label htmlFor="" className='text-gray-700 font-semibold text-lg'> Availability Timing Start </label>
          <input required onKeyDown={preventMinus} type="time"  placeholder='Enter Zip Code' onChange={(e) => setData({ ...data, availableStart: e.target.value })} className='p-2  w-full text-lg bg-transparent rounded bg-gray-200 outline-none' />
        </div>
        <div className='w-96   flex items-start flex-col justify-center p-2 m-2'>
          <label htmlFor="" className='text-gray-700 font-semibold text-lg'> Availability Timing End </label>
          <input required onKeyDown={preventMinus} type="time"  placeholder='Enter Zip Code' onChange={(e) => setData({ ...data, availableEnd: e.target.value })} className='p-2  w-full text-lg bg-transparent rounded bg-gray-200 outline-none' />
        </div>
        <div className='w-96   flex items-start flex-col justify-center p-2 m-2'>
